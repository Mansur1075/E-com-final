...
const Pagination = ({ pageNumber ,twoclick ,next ,prev ,currentPage}) => {
      ...
      <nav aria-label="Page navigation example">
        <ul className="inline-flex -space-x-px text-sm">
          ...
          {pageNumber.map((item, i) => (
            <li key={i} onClick={() => twoclick(i)}>
              <a  className={`${currentPage == i + 1 ? "flex items-center justify-center px-3 h-8 leading-tight text-[#fff] bg-[black] border-gray-300  dark:hover:text-white ]"
              : "flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300  dark:hover:text-white ]"}`}
              >
                {item}</a>
            </li>
          ))}
